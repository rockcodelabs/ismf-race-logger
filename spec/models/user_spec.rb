...
RSpec.describe User do
  ...
  describe "model is thin" do
    ...
    it "has no business logic methods" do
      ...
      # Note: admin? is auto-generated by ActiveRecord for boolean columns, so we don't check it
      # Note: display_name is a presentation helper allowed in the model for view compatibility
    ...
    it "has display_name presentation helper for views" do
      user = described_class.new(email_address: "test@example.com", name: "Test User")
      expect(user.display_name).to eq("Test User")

      user_without_name = described_class.new(email_address: "noname@example.com", name: "")
      expect(user_without_name.display_name).to eq("noname")
    end

    it "has no custom class methods beyond Rails defaults" do
      # authenticate_by is provided by has_secure_password (Rails built-in)
      # We should NOT have custom class methods like our old authenticate_by override
      # The model should only have Rails-provided methods
      expect(described_class).to respond_to(:authenticate_by) # Rails built-in from has_secure_password
    end
  end
