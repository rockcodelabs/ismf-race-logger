...
RSpec.describe Operations::Reports::Confirm do
  ...
  describe "#call" do
    context "with a pending_review report" do
      let!(:report) do
    ...
    context "with an already confirmed report" do
      let!(:report) do
      ...
      it "returns Failure with invalid_status error" do
        ...
        expect(result.failure.first).to eq(:invalid_status)
        expect(result.failure.last).to include("must be pending_review to confirm")
      end
    ...
    context "confirming multiple reports" do
      let!(:report1) do
        create(:report, :pending_review,
               race: race,
               user: user,
      ...
      it "confirms each report independently" do
        result1 = operation.call(id: report1.id)
        result2 = operation.call(id: report2.id)

        expect(result1).to be_success
        expect(result2).to be_success
        expect(result1.value!.status).to eq("confirmed")
        expect(result2.value!.status).to eq("confirmed")
      end
    end
