...
module Operations
  module Contracts
    # Validates input for creating an expenses justification
    #
    # @example
    #   contract = Operations::Contracts::CreateExpensesJustification.new
    #   result = contract.call(name: "John Doe", competition_id: 1, ...)
    #   result.success? # => true/false
    #   result.errors.to_h # => { name: ["must be filled"] }
    #
    class CreateExpensesJustification < Dry::Validation::Contract
      params do
        # References
        required(:user_id).filled(:integer)
        required(:competition_id).filled(:integer)

        # Personal Information (auto-filled from user profile, but can be overridden)
        optional(:name).filled(:string)
        optional(:address).filled(:string)
        optional(:identity_document_number).filled(:string)

        # Bank Information (auto-filled from user profile, but can be overridden)
        optional(:bank_swift).filled(:string)
        optional(:bank_iban).filled(:string)

        # Trip Details
        required(:reason_of_travel).filled(:string)
        optional(:charged_of).maybe(:string)
        optional(:place).maybe(:string)
        optional(:country).maybe(:string)
        required(:travel_start_date).filled(:date)
        required(:travel_end_date).filled(:date)
        required(:travel_days).filled(:integer)
