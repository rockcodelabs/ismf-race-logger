...
module Operations
  module Contracts
    # CreateRaceTypeLocationTemplate Contract
    #
    # Validates input parameters for creating a race type location template.
    #
    # Required fields:
    ...
    # - is_standard: boolean (defaults to false)
    # - color_code: string (must be valid ColorCode enum value)
    # - description: string
    #
    class CreateRaceTypeLocationTemplate < Dry::Validation::Contract
      params do
        ...
        optional(:is_standard).maybe(:bool)
        optional(:color_code).maybe(:string)
        optional(:description).maybe(:string)
      end

      rule(:race_type_id) do
        key.failure("must be a valid race type") unless RaceType.exists?(value)
      end
